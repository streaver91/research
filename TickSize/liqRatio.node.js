...
fs.readFile(TAQ_FILE, 'utf-8', function(err, res) {
  ...
  var outputCurFirmMonth = function() {
    ...
    var treat = (curSharesSum * TREAT_CUTOFF > curVolSum && lastDate > TREAT_START) ? 1 : 0;
    output.push([curIsin, curMon + curYear * 12, curVolSum / curAbsDailyReturnSum, treat, curVolSum / curSharesSum, curAbsDailyReturnSum, TREAT_START, lastDate].join('\t'));
  };
